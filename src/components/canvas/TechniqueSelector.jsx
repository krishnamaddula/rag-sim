...
export default function TechniqueSelector({ stage, colorScheme, activeTechniques, onToggle, onClose, onInspect }) {
  ...
  // Mobile: full-screen modal (no preview pane, larger touch targets)
  if (isMobileView) {
    return (
      <>
        {/* Backdrop */}
        ...
        {/* Full-screen modal */}
        <div
          className="fixed inset-x-2 top-16 bottom-4 z-50 rounded-2xl border border-slate-700 shadow-2xl flex flex-col"
        ...
        >
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-bold" style={{ color: color.text }}>{stage.label} Techniques</span>
            </div>
            <button onClick={onClose} className="w-10 h-10 flex items-center justify-center rounded-lg text-slate-400 hover:text-white hover:bg-slate-700 text-xl">×</button>
          </div>

          {/* Technique list — single column, no preview */}
          <div className="flex-1 overflow-y-auto py-1">
            {stage.techniques.map(tech => {
              ...
              return (
                <div
                  key={tech.id}
                  className="flex items-center gap-3 px-4 py-3.5 border-b border-slate-800/60 active:bg-slate-800/60"
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      ...
                      border: `2px solid ${isActive ? color.border : "#475569"}`,
                    }}
                  >
                    {isActive && <span className="text-xs text-white font-bold">✓</span>}
                  </div>

                  {/* Name + tagline */}
                  <div className="min-w-0 flex-1">
                    <div className="text-sm font-semibold text-slate-200">{tech.name}</div>
                    <div className="text-xs text-slate-500 leading-tight mt-0.5">{tech.tagline}</div>
                  </div>

                  {/* Details button */}
                  <button
                    className="flex-shrink-0 w-10 h-10 flex items-center justify-center rounded-lg text-slate-500 hover:text-slate-200 hover:bg-slate-700 transition-colors"
          ...
          {/* Footer */}
          <div className="px-4 py-3 border-t border-slate-800 flex items-center justify-between rounded-b-2xl flex-shrink-0">
            <span className="text-xs text-slate-600">{activeTechniques.length} selected</span>
            <button
              onClick={onClose}
